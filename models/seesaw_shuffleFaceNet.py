...
import torch
from torch.nn import Linear, Conv2d, BatchNorm1d, BatchNorm2d, PReLU, ReLU, ReLU6, Sigmoid, Dropout, MaxPool2d, \
...
class seesaw_shuffleFaceNet(Module):
    ...
    def forward(self, x):
        out = self.conv1(x) # (3,3,112,112) -> (3,64,56,56)

        out = self.conv2_dw(out) # (3,64,56,56) -> (3,64,56,56)

        out = self.conv_23(out) # (3,64,56,56) -> (3,64,28,28)

        out1 = self.conv_3(out) # (3,64,28,28) -> (3,64,28,28)
        
        out = self.conv_34(out1) # (3,64,28,28) -> (3,128,14,14)

        out2 = self.conv_4(out) # (3,128,14,14) -> (3,128,14,14)

        out = self.conv_45(out2) # (3,128,14,14) -> (3,128,7,7)

        out = self.conv_5(out) # (3,128,7,7) -> (3,128,7,7)

        out = self.conv_6_sep(out) # (3,128,7,7) -> (3,512,7,7)
